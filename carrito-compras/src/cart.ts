import { Producto, ItemCarrito } from './types.js';
 ...
 */
export function agregarAlCarrito(producto: Producto, cantidad: number = 1): boolean {
 ...
 */
export function incrementarCantidad(productoId: number): boolean {
  const item = itemsCarrito.find(item => item.producto.id === productoId);
